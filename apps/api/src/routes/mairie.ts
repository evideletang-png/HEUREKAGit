...
router.get("/gpu/probe", async (req: AuthRequest, res) => {
  ...
  for (const url of urls) {
    try {
      ...
      const raw = exec(cmd, { maxBuffer: 2 * 1024 * 1024 }).toString().trim();
      let parsed: any = null;
      try { parsed = JSON.parse(raw); } catch { parsed = raw.slice(0, 500); }
      results.push({ url, type: Array.isArray(parsed) ? "array" : typeof parsed, keys: parsed && typeof parsed === "object" && !Array.isArray(parsed) ? Object.keys(parsed) : null, length: Array.isArray(parsed) ? parsed.length : null, sample: Array.isArray(parsed) ? parsed.slice(0, 2) : parsed });
    } catch (e: any) {
