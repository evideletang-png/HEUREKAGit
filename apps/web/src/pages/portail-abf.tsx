...
export default function PortailABFPage({ params }: { params: { id?: string } }) {
  ...
  const { data: dossiersData, isLoading: loadingDossiers } = useQuery<{ dossiers: Dossier[] }>({
    ...
    queryFn: () => isDemoSessionActive()
      ...
      : apiFetch("/api/mairie/dossiers"), // Filter is handled server-side by role
  ...
  return (
    <ProfessionalShell portalType="abf" contentClassName="container mx-auto py-8 px-4 max-w-7xl w-full">
        {selectedId ? (
